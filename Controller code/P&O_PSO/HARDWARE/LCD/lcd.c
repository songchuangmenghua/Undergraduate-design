...
//数据显示
void LCD_FLOAT_Show(u16 x,u16 y,float f)
{
	int data;
	u8 count=1;
	data=f;
	...
	if(data>99){count=3;}
	if(data>999){count=4;}
	if(data>9999){count=5;}
	if(f>=0)
	{
		LCD_ShowString(x+40+count*8,y,50,16,16,".");
		LCD_ShowxNum(x+40,y,data,count,16,0);
		f-=data;
		f*=1000; // 保留3位
		LCD_ShowxNum(x+48+count*8,y,f,3,16,0X80);
	}
	else
	{
		LCD_ShowString(x+48+count*8,y,50,16,16,".");
		LCD_ShowString(x+40,y,50,16,16,"-");
		LCD_ShowxNum(x+48,y,data,count,16,0);
		f+=data;
		f*=-1000; // 保留3位
//		printf(" %f\t\n",f);
		LCD_ShowxNum(x+56+count*8,y,f,3,16,0X80);
	}
}

void LCD_Auto_Placement(u16 x_left, u16 y0, u16 dy, u16 x_right)
{
		POINT_COLOR=RED;
  	LCD_ShowString(40,y0,200,16,16,"Measurement Display");  // x = 30, y=210, width = 200, height = 16, size = 16
		LCD_ShowString(x_left,y0+dy*1,200,16,16,"Thrust:    ");		LCD_ShowString(x_right,y0+dy*1,200,16,16,"Torque:    ");
		LCD_ShowString(x_left,y0+dy*2,200,16,16,"Voltage:    ");	LCD_ShowString(x_right,y0+dy*2,200,16,16,"Current:    ");
		LCD_ShowString(x_left,y0+dy*3,200,16,16,"Epower:    ");	
	
//	LCD_ShowString(x_left,y0+dy*1,200,16,16,"Icv1:    ");	  LCD_ShowString(x_right,y0+dy*1,200,16,16,"Ucv1:    ");	
//	LCD_ShowString(x_left,y0+dy*2,200,16,16,"Icv2:    ");   LCD_ShowString(x_right,y0+dy*2,200,16,16,"Ucv2:    ");
//	LCD_ShowString(x_left,y0+dy*3,200,16,16,"Icv3:    ");	  LCD_ShowString(x_right,y0+dy*3,200,16,16,"Ucv3:    "); 
//	LCD_ShowString(x_left,y0+dy*4,200,16,16,"Icv4:    ");	 	LCD_ShowString(x_right,y0+dy*4,200,16,16,"Ucv4:    "); 
//	LCD_ShowString(x_left,y0+dy*5,200,16,16,"PmpO:    ");	  LCD_ShowString(x_right,y0+dy*5,200,16,16,"Tim:    ");
//	
//	LCD_ShowString(x_left,y0+dy*7,200,16,16,"IRR:     ");	  LCD_ShowString(x_right,y0+dy*7,200,16,16,"PTem:    ");
//	LCD_ShowString(x_left,y0+dy*8,200,16,16,"Isc:     ");	  LCD_ShowString(x_right,y0+dy*8,200,16,16,"ATem:    ");
//	LCD_ShowString(x_left,y0+dy*9,200,16,16,"Angl:    ");	  LCD_ShowString(x_right,y0+dy*9,200,16,16,"VolL:    ");
	
//	LCD_ShowString(x_left,y0+dy*8,200,16,16,"Algorithm:");
//	LCD_ShowString(x_left,y0+dy*9,200,16,16,"Time:    ");		LCD_ShowString(x_right,y0+dy*9,200,16,16,"  PD:    ");
//	LCD_ShowString(x_left,y0+dy*10,200,16,16,"Ucmd:    ");	 	LCD_ShowString(x_right,y0+dy*10,200,16,16,"Icmd:    ");
//	POINT_COLOR = GREEN;
//	LCD_ShowString(x_left-10,y0+dy*12,208,16,16,"Passive Control:keyup+key0");
//	LCD_ShowString(x_left-10,y0+dy*13,200,16,16,"Fuzzy Control:keyup+key1");
//	LCD_ShowString(x_left-10,y0+dy*14,200,16,16,"State Machine:keyup+key2");
}
